 ...
 *
 * Copied from "https://github.com/persvr/rql.git" and modified to use "item.get(property)" instead of "item[property]"
 */

({define:typeof define!="undefined"?define:function(deps, factory){module.exports = factory(exports, require("rql/parser"), require("rql/query"));}}).
define(["exports", "rql/parser", "rql/query"], function(exports, parser, QUERY){
				...
				if(jsOperator){
					...
					if (typeof target == "undefined"){
						...
						for(var i = 1;i < path.length; i++){
							...
							item +="&&item.get(" + stringify(path[0]) + ")[" + escaped.join("][") + "]";
						}
					}else{
						var item = "item&&item.get(" + stringify(path) + ")";
					}
